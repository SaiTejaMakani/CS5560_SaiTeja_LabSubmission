...
object SparkLDAMain {
  ...
  def main(args: Array[String]) {
    ...
    parser.parse(args, defaultParams).map { params =>
      run(params)
    }.getOrElse { println("error while parsing srguments <------")
  ...
  private def run(params: Params) {
    ...
    val topic_output = new PrintStream("E:\\KDM\\Tutorial 4 Source Code\\SparkOpenIE_WordNET_LDA\\data\\Results.txt")
  ...
  private def preprocess(sc: SparkContext,paths: Seq[String]): (RDD[(Long, Vector)], Array[String], Long) = {
    ...
    //Reading Stop Words
    val stopWords=sc.textFile("E:\\KDM\\Tutorial 4 Source Code\\SparkOpenIE_WordNET_LDA\\data\\stopwords.txt").collect()
